        ...
        <>
            ...
            <section className="w-full bg-white py-12 sm:py-16 lg:py-[80px]">
                <div className="mx-auto max-w-[1170px] px-4 lg:px-0">
                    <div className="grid grid-cols-1 justify-items-center gap-[30px] sm:grid-cols-2 lg:grid-cols-4">
                        {stats.map((item) => {
                            ...
                            return (
                                <div
                                    ...
                                    className={`flex h-[230px] w-full max-w-[270px] cursor-pointer flex-col items-center justify-center rounded-[4px] border transition-all ${active
                                            ? "border-[#DB4444] bg-[#DB4444] text-white shadow-lg"
                                            : "border-[#D9D9D9] bg-white text-black"
                                        }`}
                                >
                                    <div
                                        className={`flex h-[80px] w-[80px] items-center justify-center rounded-full ${active ? "bg-[#E67C7C]" : "bg-[#D9D9D9]"
                                            }`}
                                    >
                                        <div
                                            className={`flex h-[58px] w-[58px] items-center justify-center rounded-full ${active ? "bg-white" : "bg-black"
                                                }`}
                                        >
                                            <img
                                                src={item.icon}
                                                alt={item.text}
                                                className={`h-10 w-10 ${active ? "" : "brightness-0 invert"
                                                    }`}
                                            />
                                        </div>
                                    </div>

                                    <h3 className="mt-6 inter text-[28px] font-bold leading-none tracking-[0.04em] sm:text-[32px]">
                                        {item.value}
                                    </h3>

                                    <p className="mt-3 max-w-[190px] text-center poppins text-[15px] leading-[23px] sm:text-[16px] sm:leading-[24px]">
                                        {item.text}
                                    </p>
                                </div>
                            );
                    ...
                    <div className="mt-[80px] grid grid-cols-1 justify-items-center gap-[40px] md:grid-cols-2 lg:mt-[140px] lg:grid-cols-3 lg:gap-[30px]">
                        {team.map((person) => (
                            <div key={person.name} className="w-full max-w-[370px]">
                                <div className="flex h-[430px] w-full items-end justify-center bg-[#F5F5F5]">
                                    <img
                                        src={person.image}
                                        alt={person.name}
                                ...
                                <h3 className="mt-[32px] inter text-[28px] font-medium leading-[30px] tracking-[0.04em] sm:text-[32px]">
                                    {person.name}
                                </h3>
